...
export async function POST(req: NextRequest) {
  try {
    ...
    const exportRows = payrollData.map((p: any) => ({
      ...
      'Present Day': (p.attendanceStats?.presentDays || 0) + ((p.attendanceStats?.halfDays || 0) * 0.5),
      'Absent Day': p.attendanceStats?.absentDays || ((p.payroll.monthDays || 0) - (p.payroll.payableDays || 0)),
      'Paid Leaves': p.attendanceStats?.paidLeaves || 0,
      'Holiday': (p.attendanceStats?.publicHolidays || 0) + (p.attendanceStats?.paidWeekOffs || 0),
      'Total Payable Days': (p.payroll.payableDays + p.weeklyOffOTDays + (p.weeklyOffOTHalfDays * 0.5) + p.payroll.holidayOTDays + (p.payroll.holidayOTHalfDays * 0.5)).toFixed(1),
